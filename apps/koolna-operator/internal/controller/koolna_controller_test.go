...
var _ = Describe("Koolna Controller", func() {
	...
	Context("When creating Koolna", func() {
		...
		It("should create PVC, Pod, and Service", func() {
			...
			Expect(pod.Spec.Volumes[1].PersistentVolumeClaim).NotTo(BeNil())
	...
	Context("When using Spec.Images overrides", func() {
		...
		It("should reject Spec.Images.GitClone without a digest at admission", func() {
			invalid := "ghcr.io/buvis/koolna-git-clone:latest"
			koolna := &koolnav1alpha1.Koolna{
				ObjectMeta: metav1.ObjectMeta{
					Name:      resourceName,
					Namespace: "default",
			...
			Expect(err).To(HaveOccurred(),
				"CR with non-digest-pinned Spec.Images.GitClone must be rejected by CRD pattern validation")
			Expect(err.Error()).To(ContainSubstring("spec.images.gitClone"),
	...
	Describe("Volume layout", func() {
		...
		It("should not include proxy env vars on containers", func() {
			...
			for _, c := range pod.Spec.Containers {
				for _, e := range c.Env {
					Expect(e.Name).NotTo(Equal("HTTP_PROXY"), "container %s should not have HTTP_PROXY", c.Name)
					Expect(e.Name).NotTo(Equal("HTTPS_PROXY"), "container %s should not have HTTPS_PROXY", c.Name)
				}
			}
		})
