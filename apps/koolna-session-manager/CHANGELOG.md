...
- **koolna-session-manager**: install `/usr/local/bin/koolna-attach` helper that ensures the `web-remote` tmux session exists (recreating it from the persisted command at `/tmp/koolna-web-remote-cmd` if the user killed it) before attaching. Used by the webui terminal proxy.
...
- **koolna-session-manager**: koolna-attach absorbs the has-session-vs-new-session TOCTOU by falling back to `has-session` if `new-session` races against a concurrent invocation, so two near-simultaneous webui shell-button clicks land on the same `web-remote` session instead of one of them failing.
- **koolna-session-manager**: bootstrap-step label is "Creating session" (singular) instead of "Creating sessions" after the manager/worker collapse left only one tmux session.
- **koolna-session-manager**: json-escape phase strings before posting them as the `koolna.buvis.net/bootstrap-step` annotation, so callers writing arbitrary content (quote/backslash/control char) cannot corrupt the merge-patch payload.
